package dev.sussolino.juicyregions.system.reflection;


import dev.sussolino.juicyregions.JuicyRegions;
...
public class ReflectionUtil {
    ...
    public void register(String pack, Plugin plugin) {
        for (Cmd c : ReflectionUtil.getCommands(pack)) {
    ...
    @SneakyThrows
    public List<Cmd> getCommands(String packageName) {
        List<Cmd> commands = new ArrayList<>();
        for (Class<?> clazz : getClassesInPackage(packageName)) {
            if (clazz == null) continue;
            if (!clazz.isAnnotationPresent(Sussolino.class)) continue;
            commands.add((Cmd) clazz.getConstructor().newInstance());
        }
        return commands;
    }
    ...
    public List<Listener> getSpartanChecks(String packageName) {
        ...
        for (Class<?> clazz : getClassesInPackage(packageName)) {
            if (clazz == null) continue;
            if (!clazz.isAnnotationPresent(BukkitListener.class)) continue;
    ...
    public String path() throws URISyntaxException {
        return new File(JuicyRegions.class.getProtectionDomain().getCodeSource().getLocation()
                .toURI()).getPath();
    }
}